...
@section('title', 'Tambah Data Plagiat')

@section('content')
    <div class="p-6 bg-white rounded-lg shadow-md">
        <h2 class="text-lg font-semibold mb-6">Tambah Data Plagiat</h2>
        <form action="{{ route('admin.plagiat.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="nim" class="block text-gray-700">NIM</label>
                    <input type="text" name="nim" id="nim" class="form-input w-full" required>
                </div>
                <div>
                    <label for="nama" class="block text-gray-700">Nama</label>
                    <input type="text" name="nama" id="nama" class="form-input w-full" required>
                </div>
                <div>
                    <label for="id_fakultas" class="block text-gray-700">Fakultas</label>
                    <input type="text" name="id_fakultas" id="id_fakultas" class="form-input w-full" required>
                </div>
                <div>
                    <label for="id_prodi" class="block text-gray-700">Prodi</label>
                    <input type="text" name="id_prodi" id="id_prodi" class="form-input w-full" required>
                </div>
                <div>
